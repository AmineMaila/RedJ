...
public class HKeysCommand extends Command {
    ...
    public HKeysCommand(List<RespType> args) {
        super(args);
    ...
    public RespType execute(DataStore store) {
        ...
        List<RespType> items = new ArrayList<>(hash.size());
        for (var field : hash.getAll().keySet()) {
            items.add(new RespBulkString(field.bytes()));
        }
